...
(function () {
  ...
  module.factory('PushHookView', ['$onsen', '$parse', function ($onsen, $parse) {

    var PushHookView = Class.extend({
    ...
    MicroEvent.mixin(PushHookView);
    $onsen.derivePropertiesFromElement(PushHookView, ['state', 'pushDistance', 'height', 'thresholdHeight', 'disabled']);
...
(function () {
   ...
   */
  ;
  angular.module('onsen').directive('onsPushHook', ['$onsen', 'PushHookView', function ($onsen, PushHookView) {
    return {
      restrict: 'E',
      replace: false,
      scope: true,
      ...
      compile: function compile(element, attrs) {
        ...
        return {
          pre: function pre(scope, element, attrs) {
            CustomElements.upgrade(element[0]);
            var pushHook = new PushHookView(scope, element, attrs);

            $onsen.declareVarAttribute(attrs, pushHook);
            $onsen.registerEventHandlers(pushHook, 'changestate destroy');
            element.data('ons-push-hook', pushHook);

            scope.$on('$destroy', function () {
              pushHook._events = undefined;
              element.data('ons-push-hook', undefined);
              scope = element = attrs = null;
            });
          },
          post: function post(scope, element) {
